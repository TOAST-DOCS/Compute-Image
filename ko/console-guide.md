...
Linux 인스턴스의 이미지를 생성하기 전 machine-id를 초기화하여 중복을 예방하는 것을 권장합니다. 자세한 machine-id 초기화 방법 [Linux machine-id 초기화 가이드](#Linux-machineid)를 참고합니다.
...
리눅스 사용자 이미지를 만들어 해당 이미지로 인스턴스를 생성할 경우 machine-id가 중복되어 예기치 못한 문제가 발생할 수 있습니다.
