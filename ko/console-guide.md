...
이미지는 인스턴스의 기본 디스크에서 만들 수 있습니다. u2 타입의 인스턴스를 제외한 t2, m2, c2, r2, x1 타입의 인스턴스는 실행 중에 이미지 생성을 지원하지만, 데이터 정합성을 보장하지 않습니다. u2 타입의 인스턴스는 종료 상태의 인스턴스만 이미지 생성이 가능합니다. `2019.08.27 이전 버전의 Windows with MS-SQL 인스턴스는 이미지 생성을 지원하지 않습니다.`
