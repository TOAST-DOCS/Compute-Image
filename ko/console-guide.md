...
* **최대 CPU** : 해당 이미지로 인스턴스 생성 시, 최대 CPU 코어 수 입니다. 입력하지 않는 경우 최대 CPU 코어 수 제한을 사용하지 않습니다.
* **최소 CPU** : 해당 이미지로 인스턴스 생성 시, 최소 CPU 코어 수 입니다. 입력하지 않는 경우 최소 CPU 코어 수 제한을 사용하지 않습니다.
* **최소 메모리** : 해당 이미지로 인스턴스 생성 시, 최소 RAM 크기입니다. 기본 단위는 **MB**입니다. 최소 메모리 값은 필수로 입력 해야 합니다.
* **최소 블록 스토리지** : 해당 이미지로 인스턴스 생성 시, 최소 Disk 크기입니다. 기본 단위는 **GB**입니다. 최소 블록 스토리지 값은 필수로 입력 해야 합니다.
* **이미지 생성 기능 사용 여부** : 해당 이미지로 다른 이미지 생성 기능 허용 여부에 대한 정보입니다.
* **사용자 스크립트 기능 사용 여부** : 해당 이미지로 인스턴스 생성 시, 사용자 스크립트 기능 사용 여부에 대한 정보입니다.
* **사용 대상 서비스** : 해당 이미지로 인스턴스 생성 시, 서비스 별 사용 여부에 대한 정보입니다.
