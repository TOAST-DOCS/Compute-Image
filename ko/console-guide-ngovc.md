...
## Compute > Image > 콘솔 사용 가이드 { #compute-image-console-guide }
...
Windows 인스턴스의 이미지를 생성하려면 Sysprep을 이용하여 이미지 생성을 준비한 다음 인스턴스를 중지하는 것을 권장합니다. 자세한 sysprep 사용 방법은 [Windows Sysprep 가이드](#guide-for-windows-sysprep)를 참고합니다.
...
* **최대 CPU**: 해당 이미지로 인스턴스를 생성할 경우의 최대 CPU 코어 수입니다. 입력하지 않을 경우 최대 CPU 코어 수 제한을 사용하지 않습니다.
* **최소 CPU**: 해당 이미지로 인스턴스를 생성할 경우의 최소 CPU 코어 수입니다. 입력하지 않을 경우 최소 CPU 코어 수 제한을 사용하지 않습니다.
* **최소 메모리**: 해당 이미지로 인스턴스를 생성할 경우의 최소 RAM 크기입니다. 기본 단위는 **MB**입니다. 최소 메모리 값은 필수로 입력해야 합니다.
* **최소 블록 스토리지**: 해당 이미지로 인스턴스를 생성할 경우의 최소 디스크 크기입니다. 기본 단위는 **GB**입니다. 최소 블록 스토리지 값은 필수로 입력해야 합니다.
...
## 삭제 보호 설정 변경 { #change-delete-protection-settings }

<a id="deletion-protection-settings"></a>
### 삭제 보호 설정 { #deletion-protection-settings }
...
### 삭제 보호 해제 { #disable-deletion-protection }
...
## 다른 리전으로 복제 { #copy-to-another-region }
...
## 다른 프로젝트에 공유 { #share-with-other-projects }

이미지를 공유할 프로젝트를 선택하고 공유합니다.

**Compute > Image** 서비스 페이지에서 원하는 이미지를 선택한 후 **다른 프로젝트로 공유**을 클릭하여 이미지를 다른 프로젝트에 공유합니다.

> [참고] 
> 해당 기능에 대한 API 사용법은 **사용자 가이드 > Compute > Image > API 가이드**에서 **이미지 공유** 항목을 참고합니다.

<a id="guide-for-initialization-of-linux-machine-id"></a>
## Linux machine-id 초기화 가이드 { #guide-for-initialization-of-linux-machine-id }
...
## Windows Sysprep 가이드 { #guide-for-windows-sysprep }
...
### 원본 인스턴스의 이미지 버전이 2020. 08. 18. 이후인 경우 { #if-the-original-instance-image-version-is-20180818-or-later }
...
Sysprep이 실행되면 Windows 인스턴스는 자동으로 중지됩니다. NHN Cloud 콘솔에서 Windows 인스턴스의 중지를 확인하고, [이미지 생성](./console-guide-ngovc/#create-image) 기능으로 사용자 Windows 이미지를 생성합니다.
...
### 원본 인스턴스의 이미지 버전이 2020. 08. 18. 이전인 경우 { #if-the-original-instance-image-version-is-earlier-than-20180818 }
...
### Sysprep 옵션 상세 정보 { #sysprep-option-details }
...
## 실행 중인 Windows 인스턴스 이미지 생성 가이드 { #guide-to-creating-images-from-running-windows-instances }
